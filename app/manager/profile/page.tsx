...
import { User, Mail, Phone, ShieldCheck, Camera, Loader2, X, Users, Building } from "lucide-react"
...
export default function ManagerProfilePage() {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      ...
      try {
        ...
        // Fetch team size (employees under this manager)
        ...
        if (data) {
          ...
          setProfileData({
            ...
            email: data.email || "",
            phone: data.phone || "",
            state: data.state ? await getStateName(data.state) : "",
            district: data.district ? await getDistrictName(data.district) : "",
            teamSize: teamSize || 0,
          });
  ...
  const getStateName = async (stateId: number): Promise<string> => {
    const { data } = await supabase
      .from('states')
      .select('state_name')
      .eq('id', stateId)
      .single();
    return data?.state_name || "Not assigned";
  };

  const getDistrictName = async (districtId: number): Promise<string> => {
    const { data } = await supabase
      .from('districts')
      .select('district_name')
      .eq('id', districtId)
      .single();
    return data?.district_name || "Not assigned";
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-1">
            <CardContent className="pt-6 flex flex-col items-center">
              ...
              <Badge className="mt-1 bg-[#6B8E23]">{profileData.teamSize} employees</Badge>
              ...
              <div className="w-full mt-6 space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-sm">{profileData.state || "Not assigned"}</span>
                </div>
                
                <div className="flex items-center">
                  <Building className="h-5 w-5 mr-2 text-[#6B8E23]" />
                  <span className="text-sm">{profileData.district || "Not assigned"}</span>
                </div>
              </div>
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>State</Label>
                <div className="p-2 border rounded-md bg-gray-50">{profileData.state || "Not assigned"}</div>
                <p className="text-xs text-muted-foreground">State cannot be changed</p>
              </div>
              
              <div className="space-y-2">
                <Label>District</Label>
                <div className="p-2 border rounded-md bg-gray-50">{profileData.district || "Not assigned"}</div>
                <p className="text-xs text-muted-foreground">District cannot be changed</p>
              </div>

              <div className="space-y-2">
                <Label>Team Size</Label>
                <div className="p-2 border rounded-md bg-gray-50">{profileData.teamSize} employees</div>
                <p className="text-xs text-muted-foreground">Number of employees under your management</p>
              </div>
            </CardContent>
          </Card>
        </div>
